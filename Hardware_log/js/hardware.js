...
function fillCol(c) {
    if (currentTool === 'safe') return alert('Please select a tool first!');
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    table.addEventListener('mousedown', e => {
        if (currentTool !== 'safe' && e.target.closest('.c-wrap')) {
            e.preventDefault(); isMouseDown = true; updateCell(e.target.closest('.c-wrap'));
        }
    });
    table.addEventListener('mouseover', e => {
        if (isMouseDown && currentTool !== 'safe' && e.target.closest('.c-wrap')) updateCell(e.target.closest('.c-wrap'));
    });
