...
$TABLE_NAME = 'software_logs';
$PAGE_TITLE  = 'Software Logs';

// ดึง task headers — ใช้ $task_row เพื่อไม่ทับ $log ใน tbody
$task_headers = [];
$q_task = $conn->query("SELECT * FROM master_tasks WHERE system_type='software' ORDER BY CAST(SUBSTRING(column_name, 6) AS UNSIGNED) ASC");
...
// Sync equipment rows
$master_eq = $conn->query("SELECT equipment_name FROM master_equipment WHERE system_type='software'");
if ($master_eq) {
...
<head>
    ...
    <link rel="stylesheet" href="../Software_log/css/software.css">
...
<body>
    ...
    <script>
        const APP_CONFIG = {
            ...
            curM: <?=$cur_m?>,
            curY: <?=$cur_y?>,
            updateUrl: '../Software_log/update.php'
        };
    </script>
    <script src="../Software_log/js/software.js"></script>
</body>
</html>
