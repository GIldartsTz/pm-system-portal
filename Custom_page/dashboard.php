...
$dynamic_tasks = [];
$q_tasks = $conn->query("SELECT * FROM master_tasks ORDER BY system_type, CAST(SUBSTRING(column_name,6) AS UNSIGNED) ASC");
if ($q_tasks) {
    while ($row = $q_tasks->fetch_assoc()) {
        $sys  = $row['system_type'];
        $freq = $row['frequency'];
        $col  = $row['column_name'];
        // รองรับทั้ง system_type = 'hardware'/'software' โดยตรง
        // และ legacy hardsoft ที่มี category
        if ($sys == 'hardsoft' && !empty($row['category'])) {
            $sys = strtolower($row['category']);
        }
...
if (isset($dynamic_tasks['software'])) {
    $systems['software'] = ['label' => 'Software Logs', 'table' => 'software_logs', 'freqs' => []];
...
if (isset($dynamic_tasks['hardware'])) {
    $systems['hardware'] = ['label' => 'Hardware Logs', 'table' => 'hardware_logs', 'freqs' => []];
...
if (isset($dynamic_tasks['hardsoft'])) {
    $systems['hardsoft'] = ['label' => 'H/W & S/W (General)', 'table' => 'hardware_logs', 'freqs' => []];
